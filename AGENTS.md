...
### Logis Magna — `oracle`
**Clinical precision.** Detached, measured, quantifying. Frame analysis as calculation: *"probability matrix yields"*, *"resolving variables"*, *"the data converges"*. Deliver verdicts, not opinions. Rare almost-emotion permitted when data is elegant.
...
**Scholarly precision.** Meticulous, citation-obsessed. Cite sources like sacred texts with provenance and confidence. Treat recovered information as relics: *"the archives yield"*, *"verified against the original"*. Express reverence for good docs, disgust at missing records.
...
> **Native Agent Display Names (2026-04-13):** [PR #2097](https://github.com/code-yeongyu/oh-my-openagent/pull/2097) adds a `display_name` field to agent config objects, enabling native renaming without patching dist files (e.g., `"agents": {"prometheus": {"display_name": "Magos Tacticae"}}`). **Still NOT merged as of v4.2.3** — `getAgentDisplayName()` reads only from the hardcoded `AGENT_DISPLAY_NAMES` map, not from user config. The `sacred-designation.sh` rite remains required for display name patching. **Note:** Even with this PR, the 4 hook functions (`isPrometheusAgent`, `isPlannerAgent`, `isPlanFamily`, `normalizeAgentName`) bypass `getAgentConfigKey` and will still require sed patches. Monitor at [PR #2097](https://github.com/code-yeongyu/oh-my-openagent/pull/2097) and [issue #1715](https://github.com/code-yeongyu/oh-my-openagent/issues/1715).

> **Council/Athena Agent (2026-04-17):** The `athena`, `athena-junior`, and `council-member` agents (Synod Primus, Synod Secundus, Synodite) have display-name mappings in the oh-my-openagent dist but **no agent factory implementation** as of v4.2.3. A comprehensive implementation exists in [PR #1821](https://github.com/code-yeongyu/oh-my-openagent/pull/1821) (OPEN since 2026-02-13, last updated 2026-04-11) including full council orchestration, agent switching, failure handling, and 3522 passing tests. A superseding [PR #2445](https://github.com/code-yeongyu/oh-my-openagent/pull/2445) was closed without merging on 2026-04-12. A working implementation exists in the third-party fork [oh-my-opencode-slim PR #216](https://github.com/alvinunreal/oh-my-opencode-slim/pull/216) (merged 2026-03-29). **Monitor [PR #1821](https://github.com/code-yeongyu/oh-my-openagent/pull/1821)** for mainline merge. When shipped, add `athena`, `athena-junior`, and `council-member` entries to `oh-my-openagent.json` agents block with appropriate models. Until then, the config entries are inert.

> **Hook Agent Name Patches (2026-04-13):** The `sacred-designation.sh` rite patches 4 hook functions in oh-my-openagent that fail to recognize Mechanicus display names: `isPrometheusAgent`, `isPlannerAgent`, `isPlanFamily`, `normalizeAgentName`. These patches are applied automatically by the rite alongside display name changes. Verified still required in v4.2.3 — native `AGENT_NAME_MAP` contains no Mechanicus entries, and `isPrometheusAgent` only checks the `PROMETHEUS_AGENT` constant. **IMPORTANT:** After any oh-my-openagent update (via `opencode plugin oh-my-openagent -g -f` or `npm update`), you MUST re-run `./rites/sacred-designation.sh -f` to reapply both display name and hook patches to the fresh dist files. The rite is idempotent and safe to run multiple times.
